class Node:
    ...
    ----------------------
    Atributos:
        __carga: conteúdo/armazenamento do nó;
        __next: referência para o próximo nó na sequência;
        __ponteiro: referência opcional para outro nó.
    """
...
class Lista:
    ...
    def preparaPercurso(self, posicao:int):
        ...
        self.__ponteiro = self.elemento(posicao)
    ...
    def elementoCarga(self, posicao:int):
        try:
            assert not self.estaVazia(), 'Lista vazia'
            assert posicao > 0 and posicao <= len(self), f'Posicao invalida. Lista contém {self.__tamanho} elementos'
    ...
    def inserir(self, posicao:int, carga:any ):
        try:
            ...
            # CONDICAO 2: inserção na primeira posição em uma lista não vazia
            if ( posicao == 1):
    ...
    def __str__(self):
        """ Cria uma representação em string da estrutura de dados.
        """
        str = '[ '
        if self.estaVazia():
            str+= ']'
